...
from pizza_pasta_project.Pasta.pasta_product import MenuPasta, OwnPasta
import json
import time
...
class CreateMenuPasta(PastaCreator):
    ...
    def cooking_pasta(self, order):
        return MenuPasta().cooking_pasta(order)
...
class CreateOwnPasta(PastaCreator):
    ...
    def cooking_pasta(self, order):
        return OwnPasta().cooking_pasta(order)[0]

    def save_order_to_json(self, order):
        own_pasta = OwnPasta().cooking_pasta(order)[1]
        if own_pasta:
            with open(f"Pasta/orders/{round(time.time(), 2)}_{order}.json", "w", encoding='utf-8') as fp:
